...
<head>
    ...
    <title>Pasar Sentimen Crypto | Dragon Fortune</title>
...
<body>
    ...
    <div class="container">
        <header class="glass-header">
            <h1>Alternative.me</h1>
            <p>Pasar Fear & Greed Index • Dragon Fortune AI</p>
        </header>

        <!-- Filter -->
        <div class="glass-filter">
            <form action="{{ url('/sentiment') }}" method="GET">
                <div class="row g-3 align-items-end justify-content-center">
                    ...
                    <div class="col-md-2">
                        <a href="{{ url('/sentiment') }}" class="btn btn-link text-secondary text-decoration-none w-100">
                            Reset
                        </a>
                    </div>
                    @endif
        ...
        <div class="glass-card mb-5">
            <div class="d-flex justify-content-between align-items-center p-4 pb-0">
                <h5 class="m-0 fw-bold"><i class="bi bi-graph-up me-2"></i>Data Historis</h5>
                <a href="{{ url('/fetch-sentiment') }}" class="btn-glass">
                    ...
                    <table class="glass-table">
                        <thead>
                            <tr>
                                ...
                                <th class="text-end">Klasifikasi</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach($sentiments as $data)
                            @php
                            $class_slug = strtolower(str_replace(' ', '-', $data->value_classification));
                            @endphp
                            <tr>
                                <td>
                                    <div class="fw-bold">{{ \Carbon\Carbon::parse($data->api_timestamp)->format('d M Y') }}</div>
                                    <small class="text-secondary">{{ \Carbon\Carbon::parse($data->api_timestamp)->diffForHumans() }}</small>
                                </td>
                                <td>
                                ...
                                <td class="text-end">
                                    <span class="status-badge color-{{ $class_slug }}">
